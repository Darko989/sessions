...
const ActivitySection: React.FC<{ sessionId: string }> = ({ sessionId }) => {
  ...
  useEffect(() => {
    api.activity.getForSession(sessionId)
      .then((e) => setEntries(e))
      .catch(() => setEntries([]))
  }, [sessionId])
...
const PreviewChangesTab: React.FC<{ sessionId: string; baseBranch: string }> = ({ sessionId, baseBranch }) => {
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([
      api.sessions.getDiffStats(sessionId),
      api.sessions.getDiffCompare(sessionId)
    ]).then(([st, fullDiff]) => {
      setStats(st)
...
const ConflictRiskTab: React.FC<{ sessionId: string }> = ({ sessionId }) => {
  ...
  useEffect(() => {
    setLoading(true)
    api.sessions.getConflictRisk(sessionId)
      .then(setRisks)
      .catch(() => setRisks([]))
      .finally(() => setLoading(false))
  }, [sessionId])
...
export const SessionDetail: React.FC<Props> = ({ session }) => {
  ...
  const loadStatus = useCallback(async () => {
    try {
      setStatus(await api.sessions.getStatus(session.id))
  ...
  const loadChanges = useCallback(async () => {
    try {
      const [files, log] = await Promise.all([
        api.sessions.getChangedFiles(session.id),
        api.sessions.getLog(session.id)
      ])
  ...
  const loadMergeStatus = useCallback(() => {
    api.git.getBranchMergeStatus(session.worktreePath, session.branch, session.baseBranch)
  ...
  useEffect(() => {
    api.sessions.getPrUrl(session.id)
  ...
  const handleSync = async () => {
    ...
    setSyncMsg(null)
    try {
      const r = await api.sessions.refresh(session.id)
      let text: string
      if (r.hasConflicts) {
  ...
  const handlePush = async () => {
    ...
    try {
      const r = await api.sessions.push(session.id)
      if (r.success) {
        setPushMsg({ ok: true, text: 'Branch pushed to origin' })
        const url = await api.sessions.getPrUrl(session.id)
        setPrUrl(url)
        await loadStatus()
      } else {
  ...
  const handleOpenPr = () => { if (prUrl) openExternal(prUrl) }
  ...
  const toggleDiff = async (file: string) => {
    ...
    if (!diffContent[file]) {
      try {
        const diff = await api.sessions.getFileDiff(session.id, file)
  ...
  const open = async (method: string) => {
    setOpening(method)
    try { await (api.sessions as Record<string, (...args: unknown[]) => Promise<unknown>>)[method](session.id) }
    catch (err) {
  ...
  const apps: AppDef[] = [
    { id: 'openInTerminal',  label: 'Terminal',   icon: <AppIcon src={iconTerminal} label="Terminal"/>,   action: (id) => api.sessions.openInTerminal(id) },
    { id: 'openInVSCode',    label: 'VS Code',    icon: <AppIcon src={iconVSCode}   label="VS Code"/>,    action: (id) => api.sessions.openInVSCode(id) },
    { id: 'openInCursor',    label: 'Cursor',     icon: <AppIcon src={iconCursor}   label="Cursor"/>,     action: (id) => api.sessions.openInCursor(id) },
    { id: 'openInClaude',    label: 'Claude',     icon: <AppIcon src={iconClaude}    label="Claude"/>,     action: (id) => api.sessions.openInClaude(id) },
    { id: 'openInIntelliJ',  label: 'IntelliJ',   icon: <AppIcon src={iconIntelliJ}  label="IntelliJ"/>,   action: (id) => api.sessions.openInIntelliJ(id) },
    { id: 'openInCodex',     label: 'Codex CLI',  icon: <AppIcon src={iconCodex}     label="Codex CLI"/>,  action: (id) => api.sessions.openInCodex(id) },
  ]
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-2xl mx-auto px-8 py-8 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              onClick={async () => {
                ...
                setRepairMsg(null)
                try {
                  const r = await api.sessions.repairSymlinks(session.id)
                  if (r.created.length > 0) {
                    setRepairMsg({ ok: true, text: `Linked: ${r.created.join(', ')}` })
                  } else {
                    setRepairMsg({ ok: true, text: 'All files already linked (configure in Settings)' })
                  }
                } catch (err) { setRepairMsg({ ok: false, text: String(err).slice(0, 120) }) }
                finally { setRepairing(false); setTimeout(() => setRepairMsg(null), 5000) }
              }}
              disabled={repairing}
              className="ml-auto flex items-center gap-1.5 text-xs font-medium text-ink-2 border border-panel-border hover:border-ink-3 rounded-lg px-3 py-1.5 disabled:opacity-50 transition-colors"
              title="Symlink node_modules, .env, etc. from repo root"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
              {repairing ? 'Linking...' : 'Link Files'}
            </button>
            <button onClick={handleSync} disabled={syncing} className="flex items-center gap-1.5 text-xs font-medium text-white bg-accent hover:bg-accent-hover rounded-lg px-3 py-1.5 disabled:opacity-50 shadow-sm">
          ...
          {repairMsg && <div className={`mt-2 text-xs px-3 py-1.5 rounded-lg border ${repairMsg.ok ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}>{repairMsg.text}</div>}
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-panel-border overflow-hidden">
                  <div className="px-4 py-3 flex items-center gap-3">
                    ...
                    {prUrl && (
                      <button onClick={() => openExternal(prUrl)} className="text-ink-3 hover:text-ink flex-shrink-0" title="Open PR">
