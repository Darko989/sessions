  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      for (const repo of reposToFetch) {
        try {
          const branches = await api.repos.getBranches(repo.path)
...
const CreateTicketForm: React.FC<{
  ...
  useEffect(() => {
    ...
    api.tickets.fetchJiraIssueTypes(projectKey)
      .then((types) => {
    ...
    api.tickets.fetchJiraAssignableUsers(projectKey)
  ...
  const handleCreate = async () => {
    ...
    setError('')
    try {
      const ticket = await api.tickets.createJira(
...
const CreateShortcutForm: React.FC<{
  ...
  useEffect(() => {
    Promise.all([
      api.tickets.fetchShortcutProjects(),
      api.tickets.fetchShortcutWorkflowStates()
    ]).then(([p, s]) => {
  ...
  const handleCreate = async () => {
    ...
    setError('')
    try {
      const ticket = await api.tickets.createShortcut(
...
const CreateClickupForm: React.FC<{
  ...
  useEffect(() => {
    api.tickets.fetchClickupSpaces()
      .then((s) => {
  ...
  useEffect(() => {
    ...
    api.tickets.fetchClickupLists(spaceId)
      .then((l) => {
  ...
  const handleCreate = async () => {
    ...
    setError('')
    try {
      const ticket = await api.tickets.createClickup(
...
const TicketModal: React.FC<{
  ...
  useEffect(() => {
    api.tickets.fetchAll(projectKey, integration)
  ...
  const handleSearch = useCallback((value: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        if (hasJira) searches.push(api.tickets.searchJira(value, projectKey) as Promise<Ticket[]>)
        if (hasShortcut) searches.push(api.tickets.searchShortcut(value) as Promise<Ticket[]>)
        if (hasClickup) searches.push(api.tickets.searchClickup(value) as Promise<Ticket[]>)
...
export const Sidebar: React.FC = () => {
  ...
  // Auto-detect ticket integration from settings when repo doesn't have one explicitly set
  const effectiveTicketIntegration = (() => {
    if (selectedRepo?.ticketIntegration) return selectedRepo.ticketIntegration
    if (!settings) return null
    if (settings.jiraBaseUrl && settings.jiraEmail && settings.jiraApiToken) return 'jira'
    if (settings.shortcutApiToken) return 'shortcut'
    if (settings.clickupApiToken && settings.clickupTeamId) return 'clickup'
    return null
  })()
  ...
  const handleFetchOrigin = async () => {
    ...
    setFetchDone(false)
    try {
      await api.repos.fetchOrigin(selectedRepo.path)
      setFetchDone(true)
    ...
    <div className="flex flex-col h-full bg-panel-sidebar border-r border-panel-border overflow-hidden">
      ...
      <div className="px-3 pb-2 flex-shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="w-8 h-8 rounded-xl flex items-center justify-center text-white bg-accent hover:bg-accent-hover transition-colors disabled:opacity-40"
          >
            {addingRepo ? (
              <svg className="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
      ...
      <div className="px-3 pb-3 flex-shrink-0 space-y-2">
        ...
        >
          {selectedTicket ? (
            ...
            <>
              {(() => {
                const ti = selectedRepo?.ticketIntegration || effectiveTicketIntegration
      ...
      {showTicketModal && (
        <TicketModal
          ...
          integration={effectiveTicketIntegration || undefined}
          hasJira={effectiveTicketIntegration === 'jira' && !!(settings?.jiraBaseUrl && settings?.jiraEmail && settings?.jiraApiToken)}
          hasShortcut={effectiveTicketIntegration === 'shortcut' && !!settings?.shortcutApiToken}
          hasClickup={effectiveTicketIntegration === 'clickup' && !!(settings?.clickupApiToken && settings?.clickupTeamId)}
        />
      )}
    </div>
  )
